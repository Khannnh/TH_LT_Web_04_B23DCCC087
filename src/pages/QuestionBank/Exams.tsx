...
const Exams: React.FC = () => {
	...
	const columns = [
		...
		},
		{
			...
			dataIndex: 'subjectId',
			key: 'subjectId',
			render: (subjectId: string) => subjects.find((s) => s.id === subjectId)?.name || 'Không xác định',
		},
		{
			title: 'Ngày tạo',
			dataIndex: 'date',
		...
		},
		{
			...
			dataIndex: 'question',
			key: 'questionCount',
			render: (examQuestions: string[]) => examQuestions.length,
		},
		{
			title: 'Thao tác',
			key: 'action',
			render: (_: unknown, record: Exam) => (
				<Space>
					...
					<Button type='primary' danger onClick={() => handleDeleteExam(record.id)}>
						Xóa đề
					</Button>
				</Space>
			),
		...
		<Card title='Quản lý đề thi'>
			...
			>
				<Form form={form} onFinish={generateExam}>
					...
					<Form.Item name='templateExam' label='Cấu trúc đề sẵn'>
						<Select allowClear placeholder='Chọn đề thi mẫu' onChange={handleTemplateSelect}>
							{exams.map((exam) => (
