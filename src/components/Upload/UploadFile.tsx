...
const UploadFile = (props: {
  ...
  const handleChange = (val: any) => {
    ...
    const findWrongTypeFile = files?.some((file) => {
        return file?.remote !== true && !allowedTypes.includes(file.type || '');
    });
    if (findWrongTypeFile) {
        message.error('Định dạng tập tin không cho phép. Chỉ chấp nhận các định dạng: JPEG, PNG, GIF, WEBP.');
        return;
    }
